...
public class FlagEventsEnvironment implements Listener {
	...
	public void onLiquidFlow(BlockFromToEvent event) {
		Claim claim = flagManager.getClaimAt(event.getToBlock().getLocation());
		if (claim == null) return;
		if (flagManager.getFlagState(claim, "liquid_flow") == false) {
	...
	public void onExplosion(EntityExplodeEvent event) {
		...
		if (claim == null) return;
		if (flagManager.getFlagState(claim, "explosions") == false) {
			event.setCancelled(true);
	...
	public void onBlockBurn(BlockBurnEvent event) {
		...
		if (claim == null) return;
		if (flagManager.getFlagState(claim, "fire_spread") == false) {
			event.setCancelled(true);
		}
	}
	...
	public void onIceSnowForm(BlockFormEvent event) {
		...
		if (type == Material.ICE || type == Material.SNOW) {
			Claim claim = flagManager.getClaimAt(event.getBlock().getLocation());
			if (claim == null) return;
			if (flagManager.getFlagState(claim, "ice_form") == false) {
				event.setCancelled(true);
			}
		}
	}
	...
	public void onGrassSpread(BlockSpreadEvent event) {
		if (event.getSource().getType() == Material.GRASS_BLOCK) {
			Claim claim = flagManager.getClaimAt(event.getBlock().getLocation());
			if (claim == null) return;
			if (flagManager.getFlagState(claim, "grass_spread") == false) {
				event.setCancelled(true);
			}
		}
	}
